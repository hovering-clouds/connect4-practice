...
async def play(websocket,game,player,connected):
    async for message in websocket:
        event = json.loads(message)
        if not event['type']=='play':
            raise RuntimeError("error type from client.")
        try:
            row = game.play(player,event['column'])
            ret = {"type":"play","player":player,"column":event["column"],"row":row}
            for wbskt in connected:
                await wbskt.send(json.dumps(ret))
        except RuntimeError as e:
            ret = {"type":"error","message":e.args[0]}
            await websocket.send(json.dumps(ret))        
            continue
        
        if not game.winner is None:
            ret = {"type":"win","player":game.winner}
            for wbskt in connected:
                await wbskt.send(json.dumps(ret))
